...
export default function InfoTooltip({ dataset }) {
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-8 z-50 w-80 bg-[var(--panel)] rounded-2xl shadow-xl border border-[var(--border)] p-4">
          {editing ? (
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-[var(--text-dim)] uppercase tracking-wider mb-1">Título</label>
                <input
                  value={form.titulo}
                  onChange={(e) => setForm({ ...form, titulo: e.target.value })}
                  placeholder="Ex: O que é o CAGED?"
                  className="w-full border border-[var(--border)] bg-[var(--panel-2)] text-[var(--text)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-dim)] uppercase tracking-wider mb-1">Conteúdo</label>
                <textarea
                  rows={4}
                  value={form.conteudo}
                  onChange={(e) => setForm({ ...form, conteudo: e.target.value })}
                  placeholder="Explique o que este dado representa, como interpretar os valores, fontes, etc."
                  className="w-full border border-[var(--border)] bg-[var(--panel-2)] text-[var(--text)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                />
              </div>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 border border-[var(--border)] text-[var(--text-dim)] text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-[var(--panel-2)] transition-colors"
                >
              ...
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                {isGlobal && (
                  <button
                    ...
                    className="p-1 text-[var(--text-mute)] hover:text-blue-500 hover:bg-[var(--panel-2)] rounded transition-colors flex-shrink-0"
                    title="Editar"
                  >
