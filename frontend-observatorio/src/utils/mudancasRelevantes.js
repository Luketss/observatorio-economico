...
export function montarMudancas({ pib, vaf, arrecadacao, caged } = {}) {
  const comPct = [];
  if (pib?.crescimento_percentual != null && pib?.ultimo_ano) {
  ...
  if (vaf?.variacao_ipm_percentual != null && vaf?.ultimo_ano) {
  ...
  if (arrecadacao?.crescimento_percentual != null && arrecadacao?.total_geral) {
